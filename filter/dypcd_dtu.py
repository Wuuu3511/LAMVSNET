...
def check_geometric_consistency(args, depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src, src_confidence):
    ...
    for i in range(2, 11):
        ...
        mask = np.logical_and(np.logical_and(dist < 0.65 + (i-2) * 0.05, depth_diff < 0.25 + (i-2) * 0.25), sample_confi_src > 0.15)
        masks.append(mask)
...
def filter_depth(args, pair_folder, scan_folder, out_folder, plyfilename):
    ...
    for ref_view, src_views in pair_data:
        ...
        for src_view in src_views:
            ...
            sample_confi_src,\
            dist, \
            depth_diff = check_geometric_consistency(args,
                                                           ref_depth_est,
                                                           ref_intrinsics,
                                                           ref_extrinsics,
                                                           src_depth_est,
...
def dypcd_dtu_filter(args, testlist, number_worker):
