...
class BlendedMVSDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, ndepths=128, interval_scale=1.06, view_shuffle=True):
    ...
    def __getitem__(self, idx):
        ...
        if self.anti_view_noisy:
            src_train_id = src_ids[:(self.nviews - 1)//2] + src_ids[num - (self.nviews - 1)//2:]
        else:
            src_train_id = src_ids[:self.nviews - 1]

        if self.view_shuffle:
            random.shuffle(src_train_id)

        view_ids = [ref_id] + src_train_id
