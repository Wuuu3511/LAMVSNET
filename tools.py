...
def write_cam(file, cam, interval=None):
    f = open(file, "w")
    f.write('extrinsic\n')
    for i in range(0, 4):
        for j in range(0, 4):
...
def View_Right_Point(view_depth, depth_gt, mask, interval):
    #view_depth b k h w
    depth_gt = depth_gt.unsqueeze(1)
    right = torch.abs(view_depth - depth_gt) < (interval * 0.5)
    right = right.float().sum(dim=1)#b h w
    right_mask = right > 1
    right = right_mask.float()
    mean_right_num = right[mask].mean()
    return mean_right_num, right_mask

def cal_mean_right_num(depth3dcnn, depth_gt, mask, interval, view_right_mask):
    mask_sum = mask & view_right_mask
    depth_abs_right = torch.abs(depth3dcnn - depth_gt) < (interval* 0.5)
    depth_abs_right = depth_abs_right.float()
    return depth_abs_right[mask_sum].mean()
